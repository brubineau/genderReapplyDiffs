...
The units of our model is the number of first-time applicants. In these units, p women are first-time applicants and 1-p men are first-time applicants. Because the main outcome of the model -- women's share of the total applicant pool -- is the same regardless of units, we use units that help keep the model simple and clear.
...
plot(x=pEval$pRange,
     ...
     main="Effects of p parameter:\nWomen's share of first-time applicants",
     xlab="p parameter value",
     ylab="with substantive segregating effects",
...
plot(x=rEvalP1$rRange,
     y=rEvalP1$pctSub,
     ylim=c(0,1),type="l",
     main="Effects of r parameter\nWhen 0.20<b<0.99 and g>1.3",
     xlab="r parameter value",
     ylab="Portion of sampled parameter space\nwith substantive segregating effects",lwd=2,
...
abline(v=0.806,lt=2,col="gray")
text(x=0.85,y=0.25,labels="r = 0.806",srt=90,cex=1.5)
...
abline(h=0.5,lt=3,col="red")
# abline(v=c(0.618,0.804),col="gray",lt=2)
# text(x=0.63,y=0.25,labels="b = 0.618",srt=90,cex=1.5)
# text(x=0.79,y=0.25,labels="b = 0.804",srt=90,cex=1.5)

plot(x=gEvalP1$gRange,
     y=gEvalP1$pctSub,
     ylim=c(0,1),type="l",
     main="Effects of g parameter\nWhen r>0.2 and 0.20<b<0.99",
     xlab="g parameter value",
     ylab="Portion of sampled parameter space\nwith substantive segregating effects",lwd=2,
     yaxt="n",cex=1.5,cex.axis=1.5,cex.main=1.5,cex.lab=1.5)
axis(2, at=((0:4)/4), lab=paste0(((0:4)/4) * 100,"%"), las=TRUE,cex.axis=1.5,las=0)
abline(h=0.5,lt=3,col="red")
abline(v=1.84,lt=2,col="gray")
text(x=1.86,y=0.25,labels="g = 1.84",srt=90,cex=1.5)
...
```{r echo=T, results='hide', tidy=T}
png(filename = paste0("paramInterpR_",gsub("-","",Sys.Date()),".PNG"),height=2400,width=3840,res=360)
...
gridFigPctFBG <- subplot(panelsPctFBG[1:25],nrows=5,margin=0.005,
...
# favoring men in selection that yield equivalent equilibrium outcomes
gridFigORBG <- subplot(panelsORBG[1:25],nrows=5, margin=0.005,
                       shareY=T,shareX=T,titleX=F,titleY=F)
