...
#                    g=pmin(pmax(rnorm(1,mean=1.3,sd=.2),1),2)
                    ){
  sbe1 <- sbe(p,r,b,g)[1]
...
Effects of model parameters when sampling 10,000 random points in the parameter space for each x-axis value of the parameter in the figure. The observed parameter values from the three cases are given in each panel. The left column of graphs plot parameter effects in terms of the average sex bias equivalent odds ratio favoring men in selection ($OR_M$). The right column of graphs plot parameter effects in terms of the portion of sampled parameter space yielding substantial ($OR_M \geq 1.2$) segregating effects. 
...
# rowMeans(replicate(100000,prop1Sbe(b=runif(1,min=0.543,max=0.877))))
rowMeans(replicate(100000,prop1Sbe(r=runif(1,min=0.806,max=0.999))))
...
prop1EffectsG <- function(iters=1000){
  paramRange <- as.numeric(seq(1.01,2.00,0.01))
  pctSub <- sapply(paramRange,function(param){
...
### Figure A6. Effects of Individual Parameters When r>0.2, b>0.2, g>1.3 {#a6}
Effects of model parameters when sampling 10,000 random points in the parameter space for each x-axis value of the parameter in the figure. The observed parameter values from the three cases are given in each panel. The left column of graphs plot parameter effects in terms of the average sex bias equivalent odds ratio favoring men in selection ($OR_M$). The right column of graphs plot parameter effects in terms of the portion of sampled parameter space yielding substantial ($OR_M \geq 1.2$) segregating effects. 
...
r50i <- which(abs(rEvalP1$pctSub-.5)==
                       min(abs(rEvalP1$pctSub-.5)))
r50 <- rEvalP1$rRange[r50i]
r50m1 <- rEvalP1$pctSub[r50i-1]
r50p1 <- rEvalP1$pctSub[r50i+1]
r50T <- r50-0.01+(0.02*((0.5-r50m1)/(r50p1 - r50m1)))

b50i <- which(abs(bEvalP1$pctSub-.5)==
                       min(abs(bEvalP1$pctSub-.5)))
b50 <- bEvalP1$bRange[b50i]
b50m1 <- bEvalP1$pctSub[b50i-1]
b50p1 <- bEvalP1$pctSub[b50i+1]
b50T <- b50-0.02+(0.04*((0.5-b50m1)/(b50p1 - b50m1)))

g50i <- which(abs(gEvalP1$pctSub-.5)==
                       min(abs(gEvalP1$pctSub-.5)))
g50 <- gEvalP1$gRange[g50i]
g50m1 <- gEvalP1$pctSub[g50i-1]
g50p1 <- gEvalP1$pctSub[g50i+1]
g50T <- g50-0.02+(0.04*((0.5-g50m1)/(g50p1 - g50m1)))

png(filename = paste0("paramInterp3x1_",gsub("-","",Sys.Date()),".PNG"),width=2400,height=3840,res=360)

par(mar=c(4.1,6.1,4.1,0.5),mfrow=c(3,1))
...
abline(v=r50T,lt=2,col="gray")
text(x=r50T+0.015,y=0.25,
     labels=paste0("r = ",round(r50T,3),collapse=""),srt=90,cex=1.5)

plot(x=bEvalP1$bRange,
     y=bEvalP1$pctSub,
     ylim=c(0,1),type="l",
     main="Effects of b parameter\nWhen r>0.2 and g>1.3",
     xlab="b parameter value",
     ylab="Portion of sampled parameter space\nwith substantial segregating effects",lwd=2,
     yaxt="n",cex=1.5,cex.axis=1.5,cex.main=1.5,cex.lab=1.5)
axis(2, at=((0:4)/4), lab=paste0(((0:4)/4) * 100,"%"), las=TRUE,cex.axis=1.5,las=0)
# abline(v=cases$b,lt=3)
# text(x=cases$b-0.025,y=0.25,labels=cases$caseID,srt=90)
abline(h=0.5,lt=3,col="red")
abline(v=b50T,col="gray",lt=2)
text(x=b50T+0.015,y=0.25,labels=paste0("b = ",round(b50T,3),collapse=""),srt=90,cex=1.5)
# text(x=0.79,y=0.25,labels="b = 0.804",srt=90,cex=1.5)

plot(x=gEvalP1$gRange,
     y=gEvalP1$pctSub,
     ylim=c(0,1),type="l",
     main="Effects of g parameter\nWhen r>0.2 and b>0.20",
     xlab="g parameter value",
     ylab="Portion of sampled parameter space\nwith substantial segregating effects",lwd=2,
     yaxt="n",cex=1.5,cex.axis=1.5,cex.main=1.5,cex.lab=1.5)
axis(2, at=((0:4)/4), lab=paste0(((0:4)/4) * 100,"%"), las=TRUE,cex.axis=1.5,las=0)
abline(h=0.5,lt=3,col="red")
abline(v=g50T,lt=2,col="gray")
text(x=g50T+0.015,y=0.25,labels=paste0("g = ",round(g50T,3),collapse=""),srt=90,cex=1.5)
...
plot(x=rEvalP1$rRange,
     ...
     yaxt="n",cex=1.5,cex.axis=1.5,cex.main=1.5,cex.lab=1.5)
axis(2, at=((0:4)/4), lab=paste0(((0:4)/4) * 100,"%"), las=TRUE,cex.axis=1.5,las=0)
# polygon(x=c(rEvalP1$rRange[which(rEvalP1$rRange>=0.6)][1],
#             rEvalP1$rRange[which(rEvalP1$rRange>=0.6)],
#             rEvalP1$rRange[length(rEvalP1$rRange)]),
#         y=c(0,
#             rEvalP1$pctSub[which(rEvalP1$rRange>=0.6)],
#             0),
#         col="lightGray")
abline(h=0.5,lt=3,col="red")
...
gridFigPctFBG <- subplot(panelsPctFBG[1:25],nrows=5,margin=c(0,0.03,0.01,0),
        shareY=T,shareX=T,titleX=F,titleY=F)
gridFigPctFBG
# save_image(gridFigPctFBG,file="fig3.png",width=1920,height=1200,scale=1)
```
