...
The model numbers are in the terms of the number of first-time applicants. In these units, _p_ women are first-time applicants and 1 - _p_ men are first-time applicants. Because the main outcome of the model -- women's share of the total applicant pool -- has no units, we chose units that help keep the model simple and clear.

## Numeric solution
We use the R package deSolve (Soetaert, Petzoldt, & Setzer 2010) to solve our model numerically.
```{r numeric-solution, tidy=T, results='hide'}
require(deSolve)
...
appModel <- function(rounds=500,pParam,rParam,bParam,gParam){
  ...
  return(outOpen[dim(outOpen)[1],"AW"]/
           (outOpen[dim(outOpen)[1],"AW"]+
              outOpen[dim(outOpen)[1],"AM"]))
}
```
...
```{r analytic-solution, tidy=T, results='hide'}
...
```{r plot-analytic-numeric-compare, tidy=TRUE, eval=FALSE}
...
diff1k <- replicate(1000,compareSols()) # Note: may require >1hour to complete
...
```{r plot-analytic-numeric-diffs, tidy=T}
plot(density(diff1k[2,which(diff1k[4,]!=0)],na.rm=T),
     main="Difference between numeric and analytical solutions\n
     for randomly-selected points in parameter space",
     cex.main=0.75 # note almost all results 
...
```{r plot-analytic-numeric-model-time, tidy=T}
plot(x=diff1k[1,which(diff1k[4,]!=0)],
   y=diff1k[2,which(diff1k[4,]!=0)],      # plot difference as a function of model time
    ...
    ylab="Numeric - Analytic Result",type="p",
     main="Differences between numeric and\n
   analytical solutions by model time",
     cex.main=0.75)  
```
...
```{r time-to-equilibrium-function, tidy=T}
...
time2Eq <- function(maxTime){
  ...
  rRand <- runif(1,min=0.01,max=0.99) # rejection rate
  bRand <- runif(1,min=0.01,max=0.99) # rate of reapplication by rejected women
  gRand <- runif(1,min=1,max=10)      # gender difference in reapplication 
                                      # (odds ratio)
...
```{r time-to-equilibrium-10k, tidy=T}
plot(time2Eq1kMax10k[5,which(time2Eq1kMax10k[7,]!=0)],
     ...
     main="Differences between t and t+100\n
   solutions by model time to 10,000",
     cex.main=0.75 # Running time2Eq 10,000 times and using maxTime = 1,000, 
...
```{r time-to-equilibrium-1k, tidy=T}
plot(time2Eq1kMax1k[5,which(time2Eq1kMax1k[7,]!=0)],
     ...
     ylab="t and t+100 Difference",
     main="Differences between t and t+100\n
   solutions by model time to 1,000",
     cex.main=0.75)
...
```{r womens-share-at-equilibrium, tidy=T}
...
We calculate the odds ratio favoring men in selection that yields a women’s share of selected applicants equal to the women’s share of the applicant pool at equilibrium (for a particular set of model parameters) as the sex bias equivalence (of that particular set of model parameters). The code for these calculations is below.

```{r sbe-function, tidy=T}
# Sex Bias Equivalence (SBE) function
# Gives the Odds Ratio favoring men in selection that yields the same
# women's share of selected applicants as given by
...
getGs <- lapply(as.list(1:3),function(c){
  replicate(100000, # generates 100k instances of 
                    # g (odds ratio of men's higher  
                    # reapplication rate relative to women)
            ...
            # men's higher reapplication rate
            })
})
...
We consider three such potential omissions from our model: (1) not examining selected applicants explicitly, (2) omitting the possibility that the reapplication decision may be associated with applicant quality (i.e., applicant quality interactions), and (3) omitting the possibility that evaluation of applicants may be affected by reapplication status (i.e., screening interactions). Each of these omissions could plausibly influence the inferences from our model analysis. We provide a detailed examination of each omission below. For the first omission, we conclude that the equilibrium composition is not affected. For the second and third omissions, we discuss empirical results. For the quality interaction concern, we conduct a new empirical analysis. In both cases, we find no evidence for interacting mechanisms. Absent evidence for the presence of these mechanisms, we keep our model in its simplified form. 

## (1) Modeling stocks of selected applicants

### Finding: When modeling stocks of selected applicants, we find that the women's share  of those stocks has the same equilibrium as women's share of applicants.
...
# returns the final women's share of selected men and women.
selModel <- function(rounds,pParam,rParam,bParam,gParam){
  ...
  # return the final women's share of selected men and women
...
# of the applicant pool (the model in the paper) and
...
```{r variant-same-equilibrium, tidy=T, eval=T}
...
plot(x=c(500,1000,10000,100000),
     ...
     main="Women's share of applicants and\n
     women's share of selected converge",
     cex.main=1)
...
```
### Interpretation: The equilibrium of the women's share among THE SELECTED and the women's share among APPLICANTS are the same equilibrium. This is shown by how the difference between these two values goes to zero with increasing model time.

## (2) Applicant quality interactions

### Finding: After examining several empirical contexts where differential reapplication behavior by the quality of the applicant could be observed, we find no quality interactions on reapplication behaviors.

The model sets aside differences in applicant quality. This could be a problematic oversimplification if applicant quality were associated with gender differences in reapplication after rejection. For example, prior research finds that men are less likely than women to internalize negative assessments of their performance (Correll 2001). If so, then rejected men may reapply more randomly – entailing few quality differences between first-applying and reapplying men – while rejected women may reapply in a manner that more reflects the internalization of a negative assessment; the implication in this case would be that reapplying women were more different (than first-applying women) on some quality-related dimension. We searched for empirical evidence of gender differences in changes between first-time and reapplying applicants, both through our own analysis of the data from one of the three cases (i.e., crowdfunding) and in findings documented by published studies. Using the crowdfunding data, we find no evidence for gender differences in project changes when the same project was re-proposed. 

The two other published studies that we used to ground our model (viz., Brands and Fernandez-Mateo 2017; Jensen et al. 2018) likewise find no significant gender differences in the effects of applicant quality on reapplication decisions. Kolev et al. (2019) and Penner and Willer (2019) report similar evidence in the contexts of grant applications and of retaking failed STEM classes; in other words, these authors did not find that quality differences drive gender differences in reapplication. Therefore, despite the potential role for applicant quality in reapplication behavior, we follow the best available evidence from multiple empirical analyses and exclude quality differences from our model. 

In addition, we use the crowdfunding setting to perform our own analysis of the data as a means of examining our model choices regarding both applicant quality and screening effects from reapplication. Below, we describe in detail how we did so.
...
```{r fig2, tidy=T}
# Plotting 1 panel of women's share of the applicant pool at equilibrium
panPctFBG3.7 <- getPctFPanelBG(0.3,0.7) # p=0.3, r=0.7
panPctFBG3.7
```

![Figure 2](./fig2_20211105.png)
...
```{r fig3, tidy=T}
# Plotting 5 x 5 panels of women's share of the applicant pool at equilibrium
gridFigPctFBG <- subplot(panelsPctFBG[1:25],
                         nrows=5,margin=c(0,0.03,0.01,0),
                         shareY=T,shareX=T,titleX=F,titleY=F)
gridFigPctFBG
```
![Figure 3](./Fig3_PctF_20220518.png)
--------
...
# favoring men in selection that yield equivalent equilibrium outcomes
gridFigORBG <- subplot(panelsORBG[1:25],
                       nrows=5, margin=c(0,0.03,0.01,0),
                       shareY=T,shareX=T,titleX=F,titleY=F)
gridFigORBG
```
Note: Figure 4 shows half of the panel from Figure 2 and half of this panel. Both constrain the horizontal axis range for g to be from 1 to 1.5.

![Figure 5](./Fig5_20220518.PNG)
...
paramEffectsG <- function(iters=1000){
  paramRange <- as.numeric(seq(1.01,2.00,0.02))
  pctSub <- sapply(paramRange,function(param){
...
```{r fig6-plot, tidy=T, eval=FALSE}
# png(filename = paste0("paramInterp2x2_",gsub("-","",Sys.Date()),".PNG"),width=3840,height=2400,res=360)
par(mar=c(4.1,6.1,4.1,0.5),mfrow=c(2,2))
...
plot(x=pEval$pRange,
     ...
     ylab="",
          yaxt="n",cex=1.5,cex.axis=1.5,cex.main=1.5,cex.lab=1.25)
axis(2, at=((0:5)/10), lab=paste0(((0:5)/10) * 100,"%"), las=TRUE,cex.axis=1,las=0)
par(adj=1)
title(ylab="with substantial segregating effects",cex.lab=1.2)
par(adj=0.5)
...
axis(2, at=((0:5)/10), lab=paste0(((0:5)/10) * 100,"%"), las=TRUE,cex.axis=1,las=0)
par(adj=0)
title(ylab="Portion of sampled parameter space",cex.lab=1.2)
par(adj=0.5)
...
![Figure 6](paramInterp2x2_20220627.PNG)
...
prop1Sbe <- function(p=runif(1,min=0.01,max=0.49),
                    r=runif(1,min=0.2,max=0.99),
                    b=runif(1,min=0.2,max=0.99),
                    g=runif(1,min=1.3,max=2)){
  sbe1 <- sbe(p,r,b,g)[1]
  return(c(sub=as.numeric(sbe1>=1.2),
           sbe=sbe1))
}
...
prop1EffectsR <- function(iters=1000){
  paramRange <- as.numeric(seq(0.01,0.99,0.01))
  pctSub <- sapply(paramRange,function(param){
...
```{r fig7-plot, echo=T, results='hide', tidy=T}

par(mar=c(4.1,6.1,4.1,0.5),mfrow=c(1,1))

x50i <- which(abs(rEvalP1$pctSub-.5)==
                       min(abs(rEvalP1$pctSub-.5)))
x50 <- rEvalP1$rRange[x50i]
y50m1 <- rEvalP1$pctSub[x50i-1]
y50p1 <- rEvalP1$pctSub[x50i+1]
x50T <- x50-0.01+(0.02*((0.5-y50m1)/(y50p1 - y50m1)))

# png(filename = paste0("paramInterpR_",
#                       gsub("-","",Sys.Date()),".PNG"),height=2400,width=3840,res=360)
...
plot(x=rEvalP1$rRange,
     y=rEvalP1$pctSub,
     ylim=c(0,1),type="l",
     main="Effects of r parameter\nWhen b>0.20 and g>1.3",
     xlab="r parameter value",
     ylab="Portion of sampled parameter space\nwith substantial segregating effects",lwd=2,
     yaxt="n",cex=1.5,cex.axis=1.5,cex.main=1.5,cex.lab=1.5)
axis(2, at=((0:4)/4), lab=paste0(((0:4)/4) * 100,"%"), las=TRUE,cex.axis=1.5,las=0)
abline(h=0.5,lt=3,col="red")
abline(v=x50T,lt=2,col="darkGray")
text(x=round(x50T,3)+0.015,
     y=0.25,
     labels=paste0("r = ",round(x50T,3),collapse=""),
     srt=90,cex=1.5)

# dev.off()
```

![Figure 7](./paramInterpR_20220627.PNG)

-----

## Figure 8

```{r fig8, tidy=T, results='hide'}
# read in data
require(haven)
...
Correll SJ (2001) Gender and the career choice process: The role of biased self-assessments. _American Journal of Sociology_ 106(6): 1691-1730. 
...
Grout J, Hanson I, Johnson S, Kramer A, Novoseltsev A, Stein W. (2021). _SageMathCell._ https://sagecell.sagemath.org/. 

Heckman JJ (1979) Sample selection bias as a specification error. _Econometrica: Journal of the econometric society_ 153-161. 

Iacus SM, King G, Porro G (2011) Multivariate matching methods that are monotonic imbalance bounding. _Journal of the American Statistical Association_ 106(493): 345-361. 
...
Kulik CT, Olekalns M (2012) Negotiating the gender divide: Lessons from the negotiation and organizational behavior literatures. _Journal of Management_ 38(4): 1387-1415. 
...
Penner A, Willer R (2019) Men’s overpersistence and the gender gap in science and mathematics. _Socius_ 5: 1-17. 

Soetaert K, Petzoldt T, Setzer RW (2010) Solving Differential Equations in R: Package deSolve. _Journal of Statistical Software_ 33(9): 1–25. https://doi.org/10.18637/jss.v033.i09 

Wajcman J (2013) _Managing like a man: Women and men in corporate management._ John Wiley & Sons. 